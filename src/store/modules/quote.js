import { findSymbolBaseInfoList, querySymbolInfo, getEquityPremiumRate } from '@/api/trade.js'
...
import { createListByPlans } from './storeUtil.js'
    ...
    mutations: {
        ...
        add_products (state, data = []) {
            ...
            data.forEach(({ symbolId, tradeType }) => {
                ...
                if (!productMap[key]) {
                    const product = { symbolKey: key, symbolId, tradeType, cur_price: '', upDownAmount: '', upDownWidth: '' }
                    productList.push(product)
                    vue_set(productMap, key, product)
                }
            })
        },
        // 更新玩法列表
        ...
        Update_fundProduct (state, data) {
            if (state.fundInfo === null) {
                ...
                state.fundInfo = { ...state.fundInfo, ...data }
            }
        },
        // 设置行情页面玩法类型
