import { pageConfig, wpCompanyConfig, wpNav, wpSelfSymbolIndex } from '@/api/wpApi.js'
import { localSet, localGet, localRemove, getCookie, sessionSet, setCookie, isEmpty, localSetObj } from '@/utils/util.js'
...
export default {
    ...
    mutations: {
        ...
        Update_plans (state, data = []) {
            ...
            // 过滤股票玩法
            data = data.filter(el => Number(el.tradeType) !== 6)
            data.forEach(el => {
    ...
    actions: {
        ...
        // 获取渠道配置信息
        getChannelSett ({ commit, state }) {
            return pageConfig('ChannelSett').then(data => {
                if (data) {
                    const { demo_domain } = data
                    if (demo_domain) {
                    ...
                    // 设置默认区号
                    state.defaultZone = data.defaultZone
                    commit('UPDATE_wpCompanyInfo', data)
        ...
        getLoginConfig ({ state, commit, rootGetters }, pageName) {
            ...
            if (thirdLoginArr.includes('telegram')) {
                thirdLoginArr.push('telegram-pc')
            }
