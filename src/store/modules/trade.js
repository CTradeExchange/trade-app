import { queryPositionPage, queryHistoryCloseOrderList, queryPBOOrderPage, queryAbccPboPage, queryOrderPage } from '@/api/trade.js'
import CheckAPI from '@/utils/checkAPI.js'
import { cachePendingParams } from './storeUtil.js'
import { minus, divide, toFixed, plus, shiftedBy } from '@/utils/calculation.js'
    ...
    mutations: {
        ...
        Update_positionProfitLossList (state, { tradeType, list }) {
            ...
            list.forEach(({ positionId, profitLoss, previewStopPrice, riskStatus }) => {
                ...
                if (position) { 
                    vue_set(position, 'profitLoss', profitLoss)
                    vue_set(position, 'previewStopPrice', previewStopPrice)
					vue_set(position, 'riskStatus', riskStatus)
                } else {
                    vue_set(positionMap, key, { profitLoss, previewStopPrice })
                }
            })
        }
    },
    actions: {
        ...
        queryPBOOrderPage ({ dispatch, commit, state, rootState }, params) {
            ...
            if (Number(tradeType) === 9) {
                return queryAbccPboPage(params).then((res) => {
                    if (res.check()) {
