...
import { login, findCustomerInfo, logout, switchAccount, queryCustomerOptionalList, resetAccount, addCustomerOptional, queryCustomerAssetsInfo, queryAccountAssetsInfo, addCustomerOptionalBatch, findAllBizKycList, increasAccount, thirdLogin, switchUserAccount } from '@/api/user.js'

import { removeCustomerOptional } from '@/api/trade.js'
import { localSet, setToken, removeLoginParams, sessionSet, localSetObj, localGetObj, localRemoveKey, localRemove } from '@/utils/util.js'
import { vue_set, assign } from '@/utils/vueUtil.js'
import { compareAssets } from './storeUtil.js'
import { MsgSocket } from '@/plugins/socket/socket'
import { toLogin } from '@/utils/method'
...
export default {
    ...
    mutations: {
        ...
        Update_assetsInfo (state, data) {
            if (!data) return false
            vue_set(state.assetsInfo, data.tradeType, { ...data })
            const accountList = state.customerInfo?.accountList || []
            accountList.forEach(el => {
                const { currency, tradeType } = el
        ...
        Update_TgInfo (state, data) {
            state.tgLoginInfo = data
        },
        Update_BackFlag (state, data) {
            state.backFlag = data
        }
    },
    actions: {
        ...
        logout ({ dispatch, commit, state, rootState }, params = {}) {
            return logout().then(async (res) => {
                if (res.check()) {
                    ...
                    // 清除活动相关数据
                    localRemove('registerTime')
                    localRemove('RESPONSE_HEADERS_DATE')
                    commit('_activity/Update_remainingTime', 0, { root: true })
                    ...
                    if (lastAccountType === 'demo') {
                        commit('_base/UPDATE_inited', false, { root: true })
                        await vue.$init()
                    } else {
                        // 重新设置产品数据
                        await dispatch('_quote/setProductAllList', null, { root: true }).then(() => {
                            // 获取产品基础信息列表
                            commit('_quote/Update_symbolBaseLoaded', 0, { root: true })
                            dispatch('_quote/querySymbolBaseInfoList', null, { root: true })
                        })
                    }
        ...
        // 重置账户
        resetUserAccount ({ state, dispatch }, list) {
            return resetAccount().then(res => {
                if (res.check()) {
                    return res
                }
            })
