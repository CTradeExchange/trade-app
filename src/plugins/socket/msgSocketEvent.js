import { MsgWsInit } from './socket.js'
import { guid, getDevice, getToken } from '@/utils/util.js'
import { positionsTickToObj } from './socketUtil.js'
...
class SocketEvent {
    ...
    send (msgType, data, timeOut) {
        if (!this.ws || this.ws.readyState !== 1) return console.warn('消息websocket连接未准备好readyState:', this.ws?.readyState)
        const param = this.getParam(msgType, data)
        this.ws.send({ data: JSON.stringify(param) })
        if (!this.timer) {
    ...
    // 跳转到登录页面刷新
    handlerLogout () {
        console.log('websocket登出')  
        return this.$store.dispatch('_user/logout', { autoDirect: true })
    }
    ...
    subscribedListAdd (fn) {
        if (this.ws.readyState === 1) {
            ...
            console.warn('消息websocket连接未准备好readyState:', this.ws.readyState)
        }
    }
    ...
    // websocket 登录失败，登录失败只能是token失效，这时可直接给用户退出登录
    subscribe_fail () {
        console.log('websocket登录失败')
        this.handlerLogout()
    }
    ...
    initPing () {
        ...
        this.ping = setInterval(() => {
            ...
            if (ws.readyState === 1) ws.send({ data: JSON.stringify(param) })
            // if (ws.readyState === 3) {
            //     MsgWsInit({ $store: this.$store })
            // } 
        }, 10000)
    }
    ...
    notice (data) {
        ...
        if (show === 1) {
            ...
            const pages = getCurrentPages()
            const vm = pages[pages.length - 1]['$vm']
            vm.openNotice(data.content.title, data.content.text)
        }
    }
