import { QuoteWsInit } from './socket.js'
import { tickFormat, tickToObj, tick24HToObj, formatSubscribe } from './socketUtil.js'
...
class SocketEvent {
    ...
    send (cmd_id, data, timeOut) {
        if (!this.ws || this.ws.readyState !== 1) return console.warn('行情websocket连接未准备好readyState:', this.ws?.readyState)
    ...
    */
    ...
    initPing () {
        ...
        this.ping = setInterval(() => {
            if (ws.readyState === 1) this.send(14008, {})
            if (ws.readyState === 3) {
                QuoteWsInit({ $store: this.$store })
            } 
        }, 10000)
    }
}
