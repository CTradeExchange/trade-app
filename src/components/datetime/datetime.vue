<template>
    <view class='tui-datetime-picker'>
        ...
        <view class='tui-header' :class="{ 'tui-show': isShow }">
            ...
            <view v-if='unitTop' class='tui-date-header' :style='{ backgroundColor: unitBackground }'>
                ...
                <view v-if='type == 1 || type == 2 || type == 7 || type==8' class='tui-date-unit'>
                    {{ $t('calendar.day') }}
                </view>
                <view v-if='type == 1 || type == 4 || type == 5 || type == 7 || type==8' class='tui-date-unit'>
                    {{ $t('calendar.hour') }}
                </view>
                <view v-if='(type == 1 || type > 3) && type!=8' class='tui-date-unit'>
                    {{ $t('calendar.minute') }}
                </view>
                <view v-if='type > 4 && type !=8' class='tui-date-unit'>
                    {{ $t('calendar.second') }}
                </view>
            </view>
            <view class='tui-date__picker-body' :style="{ backgroundColor: bodyBackground,height:height+'rpx' }">
                <picker-view class='tui-picker-view' :value='value' @change='change'>
                    <picker-view-column v-if='!reset && (type < 4 || type == 7 || type==8)'>
                        ...
                        >
                            {{ item }}
                            <text v-if='!unitTop' class='tui-date__unit-text'>
                                {{ $t('calendar.year') }}
                            </text>
                        </view>
                    </picker-view-column>
...
export default {
    ...
    methods: {
        ...
        setHours () {
            ...
            setTimeout(() => {
                const index = this.type == 5 || this.type == 7 ? this.value.length - 3 : this.value.length - 2
        ...
        setMinutes () {
            ...
            setTimeout(() => {
                const index = this.type > 4 ? this.value.length - 2 : this.value.length - 1
        ...
        btnFix () {
            setTimeout(() => {
                ...
                const month = this.formatNum(this.month || 0)
                const day = this.formatNum(this.day || 0)
                const hour = this.formatNum(this.hour || 0)
                const minute = this.formatNum(this.minute || 0)
                const second = this.formatNum(this.second || 0)
                switch (this.type) {
                    case 1:
                        result = {
                            year: year,
