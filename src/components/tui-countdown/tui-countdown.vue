<template>
    <view class='tui-countdown-box'>
        ...
        <view v-if='days' class='tui-countdown-colon'
              :class="{ 'tui-colon-pad': borderColor == 'transparent' }" :style="{ lineHeight: colonSize + 'rpx', fontSize: colonSize + 'rpx', color: colonColor }"
        >
            {{ isColon ? ':' : '天' }}
        </view>
        <view v-if='hours'
        ...
        <view v-if='hours' class='tui-countdown-colon'
              :class="{ 'tui-colon-pad': borderColor == 'transparent' }" :style="{ lineHeight: colonSize + 'rpx', fontSize: colonSize + 'rpx', color: colonColor }"
        >
            {{ isColon ? ':' : $t('depositActivity.hour') }}
        </view>
        <view v-if='minutes'
              class='tui-countdown-item'
        ...
        <view v-if='minutes' class='tui-countdown-colon'
              :class="{ 'tui-colon-pad': borderColor == 'transparent' }" :style="{ lineHeight: colonSize + 'rpx', fontSize: colonSize + 'rpx', color: colonColor }"
        >
            {{ isColon ? ':' : $t('depositActivity.minute') }}
        </view>
        <view v-if='seconds'
              class='tui-countdown-item'
        ...
        <view v-if='seconds && !isColon' class='tui-countdown-colon'
              :class="{ 'tui-colon-pad': borderColor == 'transparent' }"
              :style="{ lineHeight: colonSize + 'rpx', fontSize: colonSize + 'rpx', color: colonColor }"
        >
            {{ unitEn ? 's' : $t('depositActivity.seconds') }}
        </view>

        <view v-if='seconds && isMs && isColon'
              class='tui-countdown-colon'
              :style="{ lineHeight: colonSize + 'rpx', fontSize: colonSize + 'rpx', color: colonColor }"
        >
            .
        </view>
        <view v-if='seconds && isMs' class='tui-countdown__ms' :style="{
        ...
        >
            <view :class="{ 'tui-ms__list': ani }">
                <view v-for='(item, index) in ms' :key='index' class='tui-ms__item' :style="{ height: height + 'rpx' }">
                    <view :class="[scale ? 'tui-countdown-scale' : '']">
                        {{ item }}
                    </view>
                </view>
            </view>
...
export default {
    ...
    created () {
        this.clearTimer()
        const seconds = Number(this.time || 0)
        if (seconds > 0) {
            this.doLoop()
        }
    },
