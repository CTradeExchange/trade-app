...
export default {
    ...
    methods: {
        ...
        saveClick () {
            ...
            this.saveCrossLevelNum(this.multipleValue)
        },
		
        // 下单保存杠杆倍数
        saveCrossLevelNum (val) {
            ...
            setCrossLevelNum({
                ...
                crossLevelNum: Number(val)
            }).then(res => {
                uni.hideLoading()
                this.$emit('update:show', false)
                this.$emit('update:multipleVal', this.multipleValue)
                this.$store.dispatch('_trade/queryPositionPage', { tradeType: product.tradeType })
                if (this.position) {
