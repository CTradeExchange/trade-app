    ...
    <view v-if='isFixed' @touchmove.stop.prevent='stop'>
        <view class='tui-bottom-popup' :class="{ 'tui-popup-show': isShow }">
            <view class='tui-calendar-header' :class="{ 'tui-calendar-radius': radius }">
                <view>{{ $t('calendar.dateSelection') }}</view>
            ...
            <view class='tui-date-box'>
                <view
                    ...
                    @tap='changeYear(0)'
                />
                <view class='tui-iconfont tui-font-arrowleft' hover-class='tui-opacity' :hover-stay-time='150' :style='{ color: monthArrowColor }' @tap='changeMonth(0)' />
                <view class='tui-date_time'>
                    {{ showTitle }}
                </view>
                <view class='tui-iconfont tui-font-arrowright' hover-class='tui-opacity' :hover-stay-time='150' :style='{ color: monthArrowColor }' @tap='changeMonth(1)' />
                <view
                    v-if='arrowType == 1'
            ...
            <view class='tui-date-content' :class="{ 'tui-flex-start': isFixed && fixedHeight }" :style="{ height: isFixed && fixedHeight ? dateHeight * 6 + 'px' : 'auto' }">
                ...
                >
                    ...
                    <view v-if='!lunar && type == 2 && startDate == `${year}-${month}-${index + 1}` && startDate != endDate' class='tui-date-desc' :style='{ color: activeColor }'>
                        {{ startText || $t('calendar.start') }}
                    </view>
                    <view v-if='!lunar && type == 2 && endDate == `${year}-${month}-${index + 1}`' class='tui-date-desc' :style='{ color: activeColor }'>
                        {{ endText || $t('calendar.end') }}
                    </view>
                </view>
            </view>
            ...
            <view class='tui-calendar-op'>
                <view class='tui-calendar-result'>
                    ...
                    <text v-if='endDate' style='margin: 0 10rpx;'>
                        {{ $t('calendar.arrive') }}
                    </text>
                    <text>{{ endDate }}</text>
                </view>
                <view class='tui-calendar-btn_box'>
                    <tui-button height='72rpx' shape='circle' :size='28' :type='btnType' @click='btnFix(false)'>
                        {{ $t('calendar.confirm') }}
                    </tui-button>
                </view>
            </view>
    ...
    methods: {
        getColor (index, type) {
            ...
            if ((this.isActiveCurrent && this.activeDate == date) || this.startDate == date || this.endDate == date) {
                color = type == 1 ? (this.$style.primary || this.activeBgColor) : this.activeColor
        ...
        changeData () {
            ...
            this.showTitle = `${this.year} ${this.$t('calendar.year')} ${this.month} ${this.$t('calendar.month')}`
        ...
        getWeekText (date) {
            ...
            return this.$t('calendar.week') + this.$t('calendar.weeks.' + week)
        },
        btnFix (show) {
            // if (!show) {
            //     this.hide()
            ...
            if (this.type == 1) {
                ...
                const diffDay = this.$dayjs(this.endDate).diff(this.$dayjs(this.startDate), 'day')
                if (diffDay > 180) {
                    return uni.showToast({
                        title: this.$t('calendar.timeLimit'),
                        icon: 'none'
                    })
                }
