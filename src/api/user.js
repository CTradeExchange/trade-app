...
import { getDevice, guid } from '@/utils/util'
import { rsaSign, token } from '@/utils/setSign' 
...
/* 注册 */
export function activityRegister (data, inviteConfig) {
    try {
        let sign = ''
        const { invitePK, inviteURL } = inviteConfig
        sign = rsaSign(data.customerNo, data.timestamp, invitePK)
        return request({
            url: inviteURL + '/app/recommended/customer/register',
            method: 'post',
            headers: {
...
/* 注销账户 */
export function destroyAccountApply (data) {
    return request({
        url: '/global/customer.app.CustomerWebApiService.destroy',
        method: 'post',
        headers: {
            version: '0.0.1'
        },
